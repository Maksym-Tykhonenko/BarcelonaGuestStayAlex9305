...
import {HotelVenuesScreen} from '../screens/HotelVenuesScreen';
import {LoaderScreen} from '../screens/LoaderScreen';
import {LocationDetailScreen} from '../screens/LocationDetailScreen';
import {NearbyLocationsScreen} from '../screens/NearbyLocationsScreen';
import {OnboardingScreen} from '../screens/OnboardingScreen';
import {OrderConfirmationScreen} from '../screens/OrderConfirmationScreen';
import {OrderReviewScreen} from '../screens/OrderReviewScreen';
import {PassScreen} from '../screens/PassScreen';
import {RoomClimateScreen} from '../screens/RoomClimateScreen';
...
function NearbyTabContent() {
  const {nearbyStack} = useAppNavigation();
  const topScreen = nearbyStack[nearbyStack.length - 1];

  if (topScreen?.name === 'LocationDetail') {
    return <LocationDetailScreen locationId={topScreen.locationId} />;
  }

  return <NearbyLocationsScreen />;
}

function ServicesTabContent() {
  const {servicesStack} = useAppNavigation();
  const topScreen = servicesStack[servicesStack.length - 1];

  if (topScreen?.name === 'VenueDetail') {
    return <VenueDetailScreen venueId={topScreen.venueId} />;
  }

  return <HotelVenuesScreen />;
}

function DiningTabContent() {
  const {diningStack} = useAppNavigation();
  const topScreen = diningStack[diningStack.length - 1];

  switch (topScreen?.name) {
    case 'OrderReview':
      return <OrderReviewScreen />;
    case 'OrderConfirmation':
      return <OrderConfirmationScreen />;
    default:
      return <DiningMenuScreen />;
  }
}
...
function TabContent() {
  ...
  switch (activeTab) {
    case 'PassTab':
      return <PassScreen />;
    case 'DiningTab':
      return <DiningTabContent />;
    case 'EventsTab':
      return <NearbyTabContent />;
    case 'ServicesTab':
      return <ServicesTabContent />;
    case 'ClimateTab':
      return <RoomClimateScreen />;
    case 'AssistTab':
